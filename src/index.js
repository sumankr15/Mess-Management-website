...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        cb(null, "C:\\Users\\LENOVO\\Desktop\\SE(F)");
    },
...
app.post("/admin/mark-absent", async (req, res) => {
    const { username, date } = req.body;

    try {
        // Mark all meals as absent and set absent flag to true
        await AttendanceCollection.updateOne(
            { username, date },
            { $set: { absent: true, breakfast: false, lunch: false, dinner: false } },
            { upsert: true }
        );

        res.status(200).json({ success: true, message: "Day marked as absent and all meals marked as absent" });
    } catch (error) {
        console.error("Error marking all meals as absent:", error);
        res.status(500).json({ success: false, message: "Error marking attendance as absent" });
    }
});
